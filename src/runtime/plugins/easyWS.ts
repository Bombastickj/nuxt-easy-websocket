...
/**
 * Nuxt plugin that provides WebSocket functionality with automatic reconnection
...
export default defineNuxtPlugin((_nuxtApp) => {
  ...
  const {
    ...
    reconnectDelay,
  } = useRuntimeConfig().public.easyWebSocket.ws
  ...
  const maxReconnectAttemptsReached = computed(() =>
    state.value.connectionAttempts >= maxReconnectAttempts,
  )
  ...
  function attemptReconnect() {
    ...
    state.value.reconnectCountdown = reconnectDelay / 1000
    const interval = 1000
    let remaining = reconnectDelay / 1000
   ...
   * Will not create a new connection if one is already open
   */
  ...
  function handleClose(event: CloseEvent) {
    ...
    if (reconnectOnClose) attemptReconnect()
  }
